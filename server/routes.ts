...
// Add a global variable to store the last uploaded filename
// This helps us create appropriate mock notes for different videos
declare global {
  namespace NodeJS {
    interface Global {
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post('/api/process-video', upload.single('video'), async (req: Request, res: Response) => {
    try {
      ...
      // Store the filename in the global variable for content detection
      global.lastUploadedFileName = fileName;
